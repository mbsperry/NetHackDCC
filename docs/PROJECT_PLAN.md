...
- [ ] **[Sonnet]** Decode a `print_glyph` batch to `{x,y,ch,color,monIdx}` via glyph bands — mechanical once offsets are known — **not started**: `shim_print_glyph` events still emit the two `glyph_info*` args as raw undereferenced pointers (see `server/driver/README.md`'s own note on this).
