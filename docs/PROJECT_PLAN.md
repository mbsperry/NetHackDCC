...
- [x] **[Opus]** `server/driver/main.c` v0: registers the shim callback, boots `nhmain()` in a per-session playground, and streams every shim event as NDJSON (fmt-driven varargs decode; ask/answer split for blocking input). Reaches the first map and first input prompt, exits cleanly on stdin EOF. Also fixed patch #2 (`hacklib.o` missing from `libnh.a`) and documented the `CHDIR`-off / `SYSCF_FILE` / lock-file / `select_menu`-returns-`-1` runtime quirks in `docs/engine-patches.md`. Build: `make -C server/driver`; verify: `sh server/driver/smoke.sh`.
- [x] **[Sonnet]** Canned answers through character creation → first map — delivered as a side effect of the v0 driver's safe-default answers (no separate code needed): the shim's no-op `player_selection` plus `select_menu`→-1 get through role/race/gender/alignment randomization with zero interactive prompts. Verified: 8 runs with `/dev/null` stdin (zero real keystrokes, varied `--name` for different random rolls) all exit 0, draw glyphs (25–59, varying with the roll), hit zero `shim_yn_function`/`shim_getlin` calls, and reach exactly one blocking input ask before EOF.
- [x] **[Sonnet]** Decode a `print_glyph` batch to `{x,y,ch,color,monIdx}` via glyph bands — mechanical once offsets are known. Added `server/driver/glyphdecode.c`, one of the translation units in this driver that includes `hack.h` (main.c stays header-free), compiled with the same defines `libnh.a` was built with so `glyph_info`/`glyph_map` struct layouts match exactly; `dcc_decode_glyph()` reads `ttychar`/`gm.sym.color` straight off the engine-populated struct and derives `monIdx` via `glyph_is_monster()`/`glyph_to_mon()` (the glyph-band macros in `include/display.h`), -1 when the glyph isn't a monster. `shim_print_glyph` events now emit `{"cb":"shim_print_glyph","w":..,"x":..,"y":..,"ch":..,"color":..,"monIdx":..}` directly. Verified: floor/wall/gold glyphs decode to `monIdx:-1`; the hero's `@` decodes to `ch:64` and a valid nonnegative `monIdx`. No engine patch needed (read-only against existing headers).
- [x] **[Sonnet]** Inject the inventory command via `cmdq_add_ec` resolved from `extcmdlist`; observe the menu event — engine's own canned-program pattern (`src/cmd.c:4688-4714`) is the template. `DCC_INJECT_INVENTORY=1` queues `ddoinv` (extcmdlist's `"inventory"` entry) onto `cmdq_add_ec(CQ_CANNED, ...)` the first time the driver is about to read a real key; `rhack()` drains it at the top of the *next* turn, ahead of any keypress, and the full menu sequence (`shim_create_nhwindow`→`shim_start_menu`→`shim_add_menu`×N→`shim_end_menu`→`shim_select_menu`) appears with no `i` ever sent. Surfaced and fixed patch #3 (`unixmain.o`/tty-windowport objects leaking into `libnh.a` on Linux, causing a `main`-symbol collision once the driver referenced enough of the archive) — see `docs/engine-patches.md`. Verify: `sh server/driver/smoke.sh` (now asserts the injection marker + menu items too).
- [x] **[Sonnet]** Emit a JSON snapshot of `u` + `gi.invent` from inside a callback. Added `server/driver/snapshot.c` (also compiled against `hack.h`); `dcc_emit_snapshot()` reads `u.ux/uy/uhp/uhpmax/uac/ulevel/uexp` and walks the `gi.invent` linked list (`invlet`/`otyp`/`quan`/`oclass`/`OBJ_NAME`) — both are plain engine globals, valid from inside *any* shim callback, not just one specific to this task. `DCC_SNAPSHOT=1` emits one `{"cb":"__snapshot",...}` line per completed turn (right before the next input ask).

**Definition of done**: a headless script walks the `@` one step east and prints correct hp/pos over JSON pipes. **Met**: `DCC_SNAPSHOT=1 DCC_KEYS=l` on a bare `--session` boot produces two `__snapshot` lines with `x` incrementing by exactly 1, `y`/`hp`/`hpmax` unchanged (`server/driver/smoke.sh`'s third check, retried across a few random rolls since an unlucky starting room can put a wall immediately east). **Phase 0 is complete.**
